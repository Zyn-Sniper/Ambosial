...
void Hooks::Init() {
	...
	// Signatures
	{
		...
		void* swingSpeed = reinterpret_cast<void*>(FindSignature("48 89 5C 24 ? 57 48 83 EC 20 48 8B 15"));
        g_Hooks.Mob_getCurrentSwingDurationHook = std::make_unique<FuncHook>(swingSpeed, Hooks::Mob_getCurrentSwingDuration);
		...
		void* InterpolatedBodyYaw = reinterpret_cast<void*>(FindSignature("48 89 5c 24 ? 55 56 57 48 83 ec ? 8b 41 ? 48 8b ea 48 8b 49")); //SMOOTH
		g_Hooks.getInterpolatedBodyYawO = std::make_unique<FuncHook>(destroySpeed, Hooks::getInterpolatedBodyYaw);

		void* InterpolatedHeadYaw = reinterpret_cast<void*>(FindSignature("48 89 5c 24 ? 55 56 57 48 83 ec ? 8b 41 ? 48 8b ea 48 8b 49")); //SMOOTH
		g_Hooks.getInterpolatedHeadYawO = std::make_unique<FuncHook>(destroySpeed, Hooks::getInterpolatedHeadYaw);
		...
		static auto bobViewHookF = [](__int64 _this, glm::mat4& matrix, float lerpT){
			...
			auto blockHitMod = moduleMgr->getModule<ItemView>();
			...
			if (blockHitMod->isEnabled()) {
				if (blockHitMod->shouldBlock) {
					if (blockHitMod->mode.selected == 1) {
					...
					if(blockHitMod->mode.selected == 2){
						lerpT = 0.f;
						matrix = glm::translate<float>(matrix, glm::vec3(0.42222223281, 0.0, -0.16666666269302368));
						matrix = glm::translate<float>(matrix, glm::vec3(-0.1f, 0.15f, -0.2f));
						javaBlockRotate(matrix, 0.25f);
					}
				}
			}
...
__int64 Hooks::RenderText(__int64 a1, MinecraftUIRenderContext* renderCtx) {
	...
	DrawUtils::setCtx(renderCtx, dat);
	{
		...
		MC_Color color = ColorUtil::ClientColor(1);
...
float* Hooks::Dimension_getFogColor(Dimension* _this, float* color, __int64 a3, float a4) {
	...
	MC_Color rColor = ColorUtil::ClientColor(1);
...
void Hooks::LoopbackPacketSender_sendToServer(LoopbackPacketSender* a, Packet* packet) {
	...
	static auto fly = moduleMgr->getModule<Fly>();
	static auto bhop = moduleMgr->getModule<Bhop>();
	...
	if (freecam->isEnabled()) {
		if (packet->isInstanceOf<C_MovePlayerPacket>() || packet->isInstanceOf<PlayerAuthInputPacket>()) {
			auto* mp = reinterpret_cast<C_MovePlayerPacket*>(packet);
			auto* pp = reinterpret_cast<PlayerAuthInputPacket*>(packet);
			if (mp->Position.x) return;
			if (mp->Position.y) return;
			if (mp->Position.z) return;
			if (pp->pos.x) return;
			if (pp->pos.y) return;
			if (pp->pos.z) return;
		}
	...
	//DesyncBoost
	if (bhop->doDesync || bhop->getPlayerAuthInputPacketHolder()->size() > 0) {
		if (TimerUtil::getCurrentMs() - bhop->desyncLastMS > bhop->desyncMs) {
			bhop->desyncLastMS = TimerUtil::getCurrentMs();
			//Send Authinput
			for (auto it : *bhop->getPlayerAuthInputPacketHolder()) {
				oFunc(a, (it));
				delete it;
				it = nullptr;
			}
			bhop->getPlayerAuthInputPacketHolder()->clear();
		} else {
			if (packet->isInstanceOf<PlayerAuthInputPacket>()) {
				moduleMgr->onSendPacket(packet);
				PlayerAuthInputPacket* meme = reinterpret_cast<PlayerAuthInputPacket*>(packet);
				bhop->getPlayerAuthInputPacketHolder()->push_back(new PlayerAuthInputPacket(*meme));  // Saving the packets
				return;
			}
		}
	} else {
		if (bhop->getPlayerAuthInputPacketHolder()->size() > 0) {
			for (auto it : *bhop->getPlayerAuthInputPacketHolder()) {
				oFunc(a, (it));
				delete it;
				it = nullptr;
	...
	// TNTFly
	if (fly->isDamaged || fly->getPlayerAuthInputPacketHolder()->size() > 0) {
		if (TimerUtil::getCurrentMs() - fly->lastFlyMs > 550) {
			fly->lastFlyMs = TimerUtil::getCurrentMs();
			// Send Authinput
			for (auto it : *fly->getPlayerAuthInputPacketHolder()) {
				oFunc(a, (it));
				delete it;
				it = nullptr;
			}
			fly->getPlayerAuthInputPacketHolder()->clear();
		} else {
			if (packet->isInstanceOf<PlayerAuthInputPacket>()) {
				moduleMgr->onSendPacket(packet);
				PlayerAuthInputPacket* meme = reinterpret_cast<PlayerAuthInputPacket*>(packet);
				fly->getPlayerAuthInputPacketHolder()->push_back(new PlayerAuthInputPacket(*meme));  // Saving the packets
				return;
			}
		}
	} else {
		if (fly->getPlayerAuthInputPacketHolder()->size() > 0) {
			for (auto it : *fly->getPlayerAuthInputPacketHolder()) {
				oFunc(a, (it));
				delete it;
				it = nullptr;
...
__int64 Hooks::ConnectionRequest_create(__int64 _this, __int64 privateKeyManager, void* a3, TextHolder* selfSignedId, TextHolder* serverAddress, __int64 clientRandomId, TextHolder* skinId, SkinData* skinData, __int64 capeData, CoolSkinData* coolSkinStuff, TextHolder* deviceId, int inputMode, int uiProfile, int guiScale, TextHolder* languageCode, bool sendEduModeParams, char a17, TextHolder* tenantId, __int64 a19, TextHolder* platformUserId, TextHolder* thirdPartyName, bool thirdPartyNameOnly, TextHolder* platformOnlineId, TextHolder* platformOfflineId, TextHolder* capeId, char a26) {
	...
	std::string randomId = std::to_string(RandomFloat(10000, 99999));
	if (moduleMgr->getModule<Franky>()->isEnabled()) inputMode = NULL;
	if (moduleMgr->getModule<AntiBan>()->isEnabled()) {
		selfSignedId->setText(randomId);
		tenantId->setText(randomId);
		platformUserId->setText(randomId);
		platformOnlineId->setText(randomId);
		platformOfflineId->setText(randomId);
	}
	TextHolder* fakeName = Game.getFakeName();
...
void Hooks::Actor__setRot(Entity* _this, Vec2& angle) {
	...
	if (_this == Game.getLocalPlayer()) {
		...
		if (killauraMod->isEnabled() && !killauraMod->targetListEmpty && killauraMod->strafe) angle = killauraMod->angle;
...
float Hooks::getInterpolatedHeadYaw(Player* _this, float a1) {
	if (_this == Game.getLocalPlayer()) {
		Packet* packet;
		if (packet && packet->isInstanceOf<PlayerAuthInputPacket>()) {
			PlayerAuthInputPacket* pkt = (PlayerAuthInputPacket*)packet;
			if (pkt) {
				headYaw = pkt->yawUnused;
			}
		}
		if (packet && packet->isInstanceOf<C_MovePlayerPacket>()) {
			C_MovePlayerPacket* pkt = reinterpret_cast<C_MovePlayerPacket*>(packet);
			if (pkt) {
				headYaw = pkt->headYaw;
			}
		}

		easedHeadYaw = Math::wrap(easedHeadYaw, headYaw - 180.f, headYaw + 180.f);
		easedHeadYaw = lerp(easedHeadYaw, headYaw, RenderUtil::getDeltaTime() * 2.07);
		return easedHeadYaw;
	}
	static auto oFunc = g_Hooks.getInterpolatedHeadYawO->GetFastcall<float, Player*, float&>();
	return oFunc(_this, a1);
}

float Hooks::getInterpolatedBodyYaw(Player* _this, float a1) {
	if (_this == Game.getLocalPlayer()) {
		easedBodyYaw = Math::wrap(easedBodyYaw, easedHeadYaw - 180.f, easedHeadYaw + 180.f);
		easedBodyYaw = lerp(easedBodyYaw, easedHeadYaw, RenderUtil::getDeltaTime() * 1.04);
		return easedBodyYaw;
	}
	static auto oFunc = g_Hooks.getInterpolatedBodyYawO->GetFastcall<float, Player*, float&>();
	return oFunc(_this, a1);
}

float* Hooks::yRot(Player* _this, float* a2, float a3) {
	static auto oFunc = g_Hooks.yRotO->GetFastcall<float*, Player*, float*, float&>();
	if (_this == Game.getLocalPlayer()) {
		Packet* packet;
		if (packet && packet->isInstanceOf<PlayerAuthInputPacket>()) {
			PlayerAuthInputPacket* pkt = (PlayerAuthInputPacket*)packet;
		...
		easedPitch = lerp(easedPitch, pitch, RenderUtil::getDeltaTime() * 2.07);
		a2[0] = easedPitch;
		return oFunc(_this, a2, a3);
	}
	return oFunc(_this, a2, a3);
...
int Hooks::Mob_getCurrentSwingDuration(void* a1) {
	auto oGetSwingDuration = g_Hooks.Mob_getCurrentSwingDurationHook->GetFastcall<int, void*>();
	auto animation = moduleMgr->getModule<ItemView>();
	if (animation->isEnabled()) return animation->swingDuration;
