...
Arraylist::Arraylist() : IModule(0, Category::VISUAL, "Display your enabled modules on the HUD!") {
	...
	addFloat("Color Speed", &cycleSpeed, cycleSpeed, 1.f, 5.f);
	addFloat("Saturation", &saturation, saturation, 0.f, 1.f);
}
...
void Arraylist::onPostRender(MinecraftUIRenderContext* renderCtx) {
	...
	for (auto it = modContainerList.begin(); it != modContainerList.end(); ++it) {
		...
		auto arrayColor = ColorUtil::ClientColor(index * (seperation * 2));
		...
		auto barrect = Vec4(rectPos.z - 1.f, rectPos.y, rectPos.z, rectPos.w);
		if (alpha > 0) {
			...
			DrawUtils::fillRectangle(barrect, arrayColor, 1.f);
			//RenderUtil::RenderImage("textures/ui/shadow.png", Vec4(barrect.x, barrect.y, 12, 12), Vec2(0, 0), Vec2(1, 1), arrayColor, saturation);
		}
		...
		//DrawUtils::drawText(Vec2(textPos.x - 1, textPos.y + 1), &textStr, MC_Color(0,0,0), textSize);
		DrawUtils::drawText(textPos.add(-1,0), &textStr, arrayColor, textSize);
