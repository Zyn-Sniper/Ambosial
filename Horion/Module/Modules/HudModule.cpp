...
HudModule::HudModule() : IModule(0, Category::CLIENT, "Displays things like the ArrayList/TabGUI.") {
	addBool("TabGui", &tabgui, tabgui);
	addBool("ClickToggle", &clickToggle, clickToggle);
	addBool("Watermark", &watermark, watermark);
	addBool("Coordinates", &coordinates, coordinates);
	addBool("Show Keybinds", &keybinds, keybinds);
	addBool("Show ArmorHUD", &displayArmor, displayArmor);
	addBool("Keystrokes", &keystrokes, keystrokes);
	addBool("Show FPS", &fps, fps);
	addBool("Show CPS", &cps, cps);
	addBool("Always show", &alwaysShow, alwaysShow);
	addFloat("Scale", &scale, scale, 0.5f, 1.5f);
}
