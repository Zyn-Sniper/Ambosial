...
void Criticals::onTick(GameMode* gm) {
	if (test) {
		...
		authInputPacket = PlayerAuthInputPacket(pos, player->getMovementProxy()->getRot().y, player->getMovementProxy()->getRot().x, player->getMovementProxy()->getRot().y);
