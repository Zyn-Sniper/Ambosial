...
IModule::IModule(int key, Category c, const char* tooltip) {
	...
	this->addInt(std::string("keybind"), &this->keybind, this->keybind, 0, 0xFF);
	this->addBool(std::string("enabled"), &this->enabled, false);
	this->ModulePos = Vec2(0.f, 0.f);
}

void IModule::addFloat(std::string name, float* floatPtr, float defaultValue, float minValue, float maxValue) {
#ifdef DEBUG
...
void IModule::addInt(std::string name, int* intPtr, int defaultValue, int minValue, int maxValue) {
#ifdef DEBUG
	if (minValue > maxValue)
		__debugbreak();  // Minimum value is bigger than maximum value
#endif
...
void IModule::addEnum(std::string name, SettingEnum* ptr) {
	SettingEntry* setting = new SettingEntry();
	setting->valueType = ValueType::ENUM_T;
	//if (0 < 0 || 0 >= ptr->GetCount())

	// Actual Value
	setting->value = reinterpret_cast<SettingValue*>(&ptr->selected);
	setting->value->_int = 0;
...
void IModule::addBool(std::string name, bool* boolPtr, bool defaultValue) {
	SettingEntry* setting = new SettingEntry();
