...
InventoryCleaner::InventoryCleaner() : IModule(0, Category::PLAYER, "Automatically throws not needed stuff out of your inventory.") {
	addBool("Tools", &keepTools, keepTools);
	addBool("Armor", &keepArmor, keepArmor);
	addBool("Food", &keepFood, keepFood);
	addBool("Blocks", &keepBlocks, keepBlocks);
	addBool("OpenInv", &openInv, openInv);
	addBool("AutoSort", &autoSort, autoSort);
}
...
bool InventoryCleaner::stackIsUseful(ItemStack* itemStack) {
	if (itemStack->item == nullptr) return false;
