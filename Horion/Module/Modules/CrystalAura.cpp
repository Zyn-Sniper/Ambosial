...
CrystalAura::CrystalAura() : IModule(VK_NUMPAD0, Category::COMBAT, "Destroys nearby End Crystals.") {
	...
	addInt("Crystal Range", &cRange, cRange, 1, 15);
	addInt("Place Range", &eRange, eRange, 1, 5);
	addInt("Player Range", &pRange, pRange, 1, 10);
	addBool("Autoselect", &AutoSelect, AutoSelect);
	addBool("Autoplace", &autoplace, autoplace);
	addBool("Enhance Slace", &pEnhanced, pEnhanced);
	addBool("Enhance Destroy", &dEnhanced, dEnhanced);
	addBool("Preview", &Preview, Preview);
	delay = 0;
}
