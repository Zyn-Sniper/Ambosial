...
void initializeSigs() {
	...
	tess_vertex = reinterpret_cast<tess_vertex_t>(FindSignature("SU CK MY DI CK NI GG ER"));
	meshHelper_renderImm = reinterpret_cast<meshHelper_renderImm_t>(FindSignature("40 f4 48 8D ac 24 ? ? ? ? 48 81 EC ? ? ? ? 49 8B F0"));
...
void DrawUtils::drawText(const Vec2& pos, std::string* textStr, const MC_Color& color, float textSize, float alpha, Fonts font) {
	...
	renderCtx->drawText(fontPtr, posF, &text, color.arr, alpha, 0, &TextMeasureData(&textSize, true), &caretMeasureData);
}
...
void DrawUtils::drawImage(std::string filePath, Vec2& imagePos, Vec2& imageDimension, Vec2& idk, Vec2& idk2, MC_Color flushColor, float alpha) {
...
void DrawUtils::drawGlow(const Vec4& pos, const MC_Color& col, float alpha, int layers, float blurRadius) {
	float dAlpha = alpha / layers;
	for (int i = 0; i < layers; i++) {
		float rrayerAlpha = alpha - dAlpha * i;
		float rayerRadius = blurRadius + (blurRadius / layers) * i;  // nig

		DrawUtils::setColor(col.r, col.g, col.b, rrayerAlpha);

		Vec4 layerbozo = pos;
		layerbozo.x -= rayerRadius;
		layerbozo.y -= rayerRadius;
		layerbozo.z += rayerRadius;
		layerbozo.w += rayerRadius;
